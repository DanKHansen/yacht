object Yacht:
   def score(dice: List[Int], category: String): Int =
      ...
      category match
         ...
         case "full house"            =>
            if dice.groupBy(identity).values.map(_.size).toSeq.sorted == Seq(2, 3) then dice.sum else 0
         case "four of a kind"        =>
            dice.groupBy(identity).collect { case (_, xs) if xs.size >= 4 => xs.take(4).sum }.maxOption.getOrElse(0)
